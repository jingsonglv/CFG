...
def swing_m(G, v1, v2):
    w_list = sorted(nx.common_neighbors(G, v1, v2))
    pair_list = list(itertools.combinations(w_list, 2))
    swing_value = 0.0
    if G.has_edge(v1, v2):
        swing_value = 1.0
    alpha = 1.0
    for (w1, w2) in pair_list:
        swing_value += float(1.0) / float(alpha + len(sorted(nx.common_neighbors(G, w1, w2))))
    return swing_value
